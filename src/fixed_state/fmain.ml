...
let choose_to_prove bdd output_file visualize_addr input_file = 
	try
		...
		| (true, None, "") -> Fprover_bdd.prove_model modl5
		| (false, None, "") -> 
		...
		| (_, None, _) ->
			if visualize_addr <> "" then begin
				...
				flush stdout;
				Fprover_visualization.prove_model modl5 visualize_addr
			end else begin
	...
	with Parsing.Parse_error -> 
		print_endline ("parse error at line: "^(string_of_int (!(Flexer.line_num))));
		print_endline "Maybe you are using the finite version of SCTLProV. Try using \"make all\" or \"make no-bcg\" to recompile the program."
